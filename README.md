...
- 모바일에서 `의뢰하기` 모달이 화면 위아래로 덜 잘리도록 레이아웃을 압축했습니다.
- 모바일 문의 모달에서는 장식 패널을 숨기고 입력칸, 제목, 버튼 간격을 줄여 폼이 한눈에 더 잘 들어오도록 조정했습니다.

### Build 0.4.0 - 2026-04-25

이번 업데이트에는 아래 내용이 반영되었습니다.

- 모바일 `WORKS` 카드에서 프로젝트 제목과 카테고리가 항상 보이도록 표시 방식을 개선했습니다.
- 키보드 사용자를 위해 필터 버튼, 문의 모달, 영상 모달의 포커스 표시를 보강했습니다.
- 모션 감소 설정을 사용하는 환경에서는 주요 애니메이션이 줄어들도록 조정했습니다.
- 의뢰 API는 디스코드와 메일 전송을 독립 처리하고, 한 채널 실패가 다른 채널 전송을 막지 않도록 개선했습니다.
- 의뢰 API 입력값 길이 제한, JSON 오류 처리, 이메일 형식일 때만 `replyTo` 적용하는 검증을 추가했습니다.
- `NEXT_PUBLIC_SITE_URL` 끝에 `/`가 있어도 sitemap, robots, canonical URL이 안정적으로 생성되도록 정리했습니다.
- 영산대 AI 웹툰 강의 갤러리 수를 현재 파일 상태에 맞춰 3장으로 정정했습니다.
- 작업자용 `AGENTS.md`, 구조 설명 `ARCHITECTURE.md`, 운영 안전 규칙 `SECURITY.md`를 추가했습니다.
...
- 개발 서버를 Windows 한글 경로에서도 안정적으로 실행할 수 있도록 `npm run dev`를 `webpack` 모드로 전환했습니다.
- 모바일 `WORKS` 그리드는 최적화를 위해 카드 모션과 영상 자동재생을 끄도록 조정했습니다.
- `WORKS` 맨 마지막에 강의/행사 이미지 3건을 추가했습니다.
- 새 강의/행사 3건은 내부 상세 페이지로 연결했고, 갤러리 이미지는 경북 교육자 대상 AI 역량강화 강의 5장, 영산대 AI 웹툰 강의 3장, RISE 사업특수효과(VFX) 해커톤 및 AI 상영 6장으로 반영했습니다.
- 르플레르 구강 스프레이 상세페이지 갤러리는 파일명 숫자 기준으로 정렬해 노출 순서가 안정적으로 유지되도록 보정했습니다.

### Build 0.2.0 - 2026-04-13

이번 업데이트에는 아래 내용이 반영되었습니다.

- `WORKS`에 고려대학교 봉사단 홍보 영상이 추가되었습니다.
- `WORKS`에 리베리 의원 강남점 스크린 영상이 추가되었습니다.
- 정성곳간 프리미엄 총각김치 상세페이지가 추가되었습니다.
- 르플레르 구강 스프레이 상세페이지가 추가되었습니다.
- 신규 영상 2건은 홈 포트폴리오용 `WEBM` 미리보기로 반영되었습니다.
- 상세페이지 갤러리 이미지는 원본보다 확대되지 않도록 표시 방식을 조정했습니다.

### Build 0.1.0 - 2026-04-13

이번 업데이트에는 아래 내용이 반영되었습니다.

- 홈페이지 기준 버전 표기를 `0.1.0`부터 시작하도록 정리했습니다.
- 팀 공유용 업데이트 기록 방식이 추가되었습니다.
- 커밋/푸시 전에 변경 내용을 같은 형식으로 확인할 수 있도록 정리했습니다.

### 작성 예시

아래 형식을 복사해서 다음 업데이트 때 그대로 쓰면 됩니다.

```md
### Build 0.4.0 - 2026-04-24

이번 업데이트에는 아래 내용이 반영되었습니다.

- 홈 메인 배너 문구가 수정되었습니다.
- 의뢰하기 모달에 새 문의 항목이 추가되었습니다.
- 포트폴리오 영상 2건이 새로 업데이트되었습니다.
```
...
- `AGENTS.md`: 작업자가 먼저 읽을 운영 원칙과 완료 전 체크리스트
- `ARCHITECTURE.md`: 라우트, 데이터 흐름, 에셋 구조, 문의 API 구조 설명
- `SECURITY.md`: 환경 변수, 문의 API, 자산/저작권, 외부 링크 안전 규칙
...
- 모바일에서는 최적화를 위해 홈 그리드 영상 미리보기를 자동재생하지 않습니다.
...
- 현재 버전은 `0.4.1`입니다.
- 업데이트 이력은 루트의 `CHANGELOG.md`에 `## Build <version> - <date>` 형식으로 기록합니다.
- 커밋/푸시 전에 `npm run update:preview`를 실행하면 현재 버전 기준 업데이트 형식을 바로 확인할 수 있습니다.
- 다음 배포나 기능 묶음 작업 전에는 `package.json`의 `version`을 올리고 `CHANGELOG.md`에 같은 버전으로 항목을 추가합니다.
- 팀원에게 바로 보여줄 공지성 내용은 이 README의 `최근 업데이트` 섹션에 같은 버전으로 함께 적습니다.
